  ...
  useEffect(()=>{
    ...
    const fetchPublicProject = async()=>{
      ...
      setProject(data)
        ...
        setError(err?.response?.data?.error || "This website is not available or not published yet.");
      }
      finally{
        setLoading(false)
      }
...
if(error || !project){
  return (
    ...
    px-4 text-center'>
      ...
      leading-relaxed mb-6'>{error}</p>
      <div className='text-[10px] font-semibold uppercase 
      tracking-widest text-zinc-400'>BuilderAI</div>
    </div>
  )
}
